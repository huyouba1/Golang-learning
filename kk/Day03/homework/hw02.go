...
func main() {
	...
	for _, ch := range article {
		if ch >= 'a' && ch <= 'z' || ch >= 'A' && ch <= 'Z' { // 通过ASCALL码进行遍历
			stat[ch]++
		}
	}
	fmt.Println(stat)

	statSlice := make([][]int, 0, len(stat))

	for ch, cnt := range stat {
		fmt.Printf("%c : %d \n", ch, cnt) // %c 字符型. 可以把输入的数字按照ASCII码相应的转换为对应的字符, %d 一个十进制数值(基数为10).
		statSlice = append(statSlice, []int{int(ch), cnt})
	}

	fmt.Println(statSlice)
	sort.Slice(statSlice, func(i, j int) bool {
		return statSlice[i][1] > statSlice[j][1]
	})
	fmt.Println(statSlice[:10])
	//fmt.Println(statSlice)

	for index, value := range statSlice[:10] {
		//value[0]
		//value[1]
