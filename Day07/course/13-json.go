...
type User struct {
	...
	Sex      bool   `json:"sex,omitempty"` // n20ame,omitempty,string   // name,type
	Birthday *Birthday
}
...
func main() {
	...
	users := []User{
		{1, "kk", "123213", true, &now},
		{2, "cac", "123213", true, &now},
		{3, "huyouba1", "123213", false, &now},
	}
	bytes, _ := json.MarshalIndent(users, "", "\t")
	fmt.Println(string(bytes))

	var birthday Birthday = now
	ctx, _ := json.Marshal(birthday)
	fmt.Println(string(ctx))

	var bt Birthday
	json.Unmarshal([]byte(`"2020-01-05 10:20:10"`), &bt)
